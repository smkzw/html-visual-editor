...
## 0.2 v4.3.3（2026-09-13 晚）— 用户实测第三轮

| 缺陷 | 根因 | 修复 |
|------|------|------|
| 页面"全部变成文字形式"（无样式裸文字） | 引擎发送响应后**立即 conn.cancel()**——`.contentProcessed` 仅代表内核收下字节，149KB 的 CSS 在缓冲排空前被 RST 截断，WebKit 拿到残缺样式表直接丢弃（小文件一直正常掩盖了此问题） | 三条发送路径统一「发送完成 → 延迟 0.25s 排空 → 关闭」；实测 149KB CSS 三轮逐字节完整 |
| CSS/JS 请求整批 403（同症状叠加因素） | WKWebView 子资源请求 **不带任何 cookie/header**（日志铁证 `cookie=-`），会话切换后旧 cookie 全灭 | **token 改走 URL 路径段** `/api/live/<token>/<rel>`，注入的 `<base>` 使所有相对引用自动携带；cookie/header 校验保留于 API POST |
| 点击导航后滚动无后续内容 | 滚动叙事站（逐屏 section）被 detectPPT 判为幻灯片，**编辑态隐藏了其余页** | detectPPT 只标记+上报；隐藏行为移入演示模式（present true/false 切换可见性），编辑态全部可见可滚动 |
| App 图标白边 | PNG 圆角方块外为实心白角 + 右下「AI生」水印 | 边界 flood-fill 透明化 + 右下角同行镜像复制去水印（渐变无损） |
